...
#define FRAME_TASK_PRO                  3          // Поток, пытающийся патрулировать границу, имеет тот же приоритет, что и поток lvgl
...
#if defined(USE_DelayUntil)
    vTaskDelayUntil(&xLastWakeTime, xDisplayFrequency); //Используйте относительную задержку для обеспечения точного времени
#else
        vTaskDelay(5); // 5ms Абсолютная задержка
#endif   
    }
}
...
static void mks_page_data_updata(void) { 
    ...
    if(mks_ui_page.mks_ui_page == MKS_UI_PAGE_LOADING) {
    ...
    else if (mks_ui_page.mks_ui_page == MKS_UI_Ready) {  //Обновляйте данные только на текущей странице
    ...
    else if(mks_ui_page.mks_ui_page == MKS_UI_Pring) { // Обновление данных интерфейса гравировки
    ...
    else if(mks_ui_page.mks_ui_page == MKS_UI_inFile) { // Обновление данных интерфейса гравировки
    ...
    else if(mks_ui_page.mks_ui_page == MKS_UI_Control) {  //Интерфейс управления
        ...
        if((mks_wifi.wifi_scanf_status == wifi_scanf_begin) || (mks_wifi.wifi_scanf_status == wifi_scanf_waitting)) {
            ...
            if(mks_grbl.wifi_connect_status == true) {          // Статус подключения
                if(mks_get_wifi_status() == true) {             // Подтвердите подключение
                    mks_lv_clean_ui();
                    mks_draw_wifi();
                    mks_wifi.wifi_scanf_status = wifi_none;
                }
            ...
            if(mks_get_wifi_status() == false) {  // Подтвердите отключение
                mks_wifi.wifi_scanf_status = wifi_scanf_begin;   // Повторное сканирование
                mks_lv_clean_ui();
                mks_draw_wifi();
            }
        }
        ...
        if(mks_updata.updata_flag == UD_UPDATA_ING) {
            ...
            // Всплывающее окно завершения обновления
            ...
            // Всплывающее окно с ошибкой обновления
...
IRAM_ATTR void frame_task(void *parg) {
    ...
    // Определите возвращаемое значение семафора
    ...
    while(1) {  
        ...
        if(sem_receive == pdTRUE) {
            ...
            vTaskSuspend(lv_disp_tcb); // Ожидающая решения задача da
            mks_run_frame(frame_ctrl.file_name);
            vTaskResume(lv_disp_tcb);   // Задача восстановления
        }
    }
}
